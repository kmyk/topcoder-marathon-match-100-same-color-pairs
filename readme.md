...
-   なぜ上手くいったのかは不明。まあ私は普段からLUCK極振りみたいなところあるし今回も幸運でしたね。
-   githubのreadmeに日記書いて画像の埋め込みもして走るのは良かった気がする。[threepipes/GifWriter.java](https://gist.github.com/threepipes/19d764e35decad6789722b32b58bb1bd)には感謝。
    ...
    実際、貪欲実装に上手く復帰できたのはこれのおかげだろう。
-   readme.mdに書くという方針は改善の余地がある。
    `git stash` を叩いたとき書きかけのreadme.mdまで持っていかれるの面倒だった。commit logが流れるし。
    gitでは管理せずissueに書くのが思い付く。しかし手元に残らないのが好きでないし、変に構造化されてしまって書きにくく、特に見返す頻度が下がるため不適。
...
-   計算資源借りて調整するのは正解だった。
    ...
    計測結果の管理が雑だったので改善したい。
-   計測の話とreadmeの話をまとめるに、必要なのはdashboard。
    ...
    実装が必要という点を除けば、blog機能も足してしまってreadmeに書いてた日記をこちらに移してしまうのが正しい気がする。

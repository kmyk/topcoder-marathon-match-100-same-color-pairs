...
| Handle   | Score       | Rank  | Last Submission Time | Language | Example Tests  | Submissions | Current Rating | Predicted Rating  | Rating Delta  |
|----------|-------------|-------|----------------------|----------|----------------|-------------|----------------|-------------------|---------------|
| eldidou  | 819329.71   | 1     | 04.22.2018 09:42:47  | C++      | 3              | 4           | 2560           | 2641.72           | 81.72         |
| kimiyuki | 819329.71   | 1     | 04.25.2018 14:33:17  | C++      | 38             | 14          | 1745           | 2049.62           | 304.62        |
| wleite   | 819329.71   | 1     | 04.25.2018 03:45:03  | Java     | 21             | 18          | 2708           | 2754.93           | 46.93         |
| hakomo   | 818967.39   | 4     | 04.25.2018 09:52:37  | C++      | 3              | 4           | 2281           | 2370.47           | 89.47         |
| imazato  | 818949.28   | 5     | 04.25.2018 13:14:56  | C++      | 16             | 12          | 2408           | 2455.01           | 47.01         |

`b737b223ab76f14c06e177cf34dc314e0b84aad3` は seed < 10000のC = 6に関し総和48 総時間4300秒ぐらいが平均。
seed = 4165 については未だあまり成功しない。実質敗北みたいなところはある。
まだ追加で回してるパラメタ調整が当たりを引くことを祈るばかり。

| seed | H   |  W     | C     | remaining | elapsed |
|------|-----|--------|-------|-----------|---------|
| 1592 | 86  |  80    | 6     | 6         | 9.30893 |
| 1694 | 62  |  41    | 6     | 4         | 9.30339 |
| 4165 | 12  |  13    | 6     | 4         | 9.30031 |
| 5398 | 16  |  67    | 6     | 4         | 9.30123 |
| 8109 | 90  |  13    | 6     | 6         | 9.30225 |
| 8882 | 58  |  59    | 6     | 4         | 9.30854 |
| 9270 | 88  |  94    | 6     | 4         | 9.31099 |
| 9307 | 48  |  97    | 6     | 8         | 9.30646 |
| 9982 | 100 |  100   | 6     | 8         | 9.31369 |
...
-   なぜ上手くいったのかは不明。たぶん運が良かった。まあ運も実力のうちだし勝ちは勝ち。
    ...
    最後に残ったマスを優先的に消したのは非自明かつ有効だったように思う。偶然SATの話を授業で聞いてたので気付けたのだしたぶんこれも幸運。
-   githubのreadmeに日記書いて画像の埋め込みもして走るのは良かった気がする。[threepipes/GifWriter.java](https://gist.github.com/threepipes/19d764e35decad6789722b32b58bb1bd)には感謝。
    全部ひとつにまとめてあって簡単に見返せるのはよい。「最初は気付いていたんだけど途中から忘れてしまっていた」みたいなのが消える。
    実際、はずれ方針から貪欲実装に上手く復帰できたのはこれのおかげだろう。
    readme.mdに置くのはすこし見えすぎる気がするので名前を変えておきたい。技術的事実以外の長文を書くとたいてい楽しい結果にならない。
    特に今回は順位が良かったので調子にのって不要のことを書いているため。例えばまさにこの部分。
    「私は天才」みたいな気分で居ながらコミュニケーションを面倒がるのは死しか待ち受けていないが、残りの人生が穏やかに過ごせるのならもうそれでいい。
-   他への影響は今まで通り。ヒントをくれたSATの授業も犠牲となった。しかし授業自体は既知でつまらないためMMがなくても行ってないはず。先生と直接話すと面白いのだが。
    学生を馬鹿にしてないまともな授業がひとつだけあるのでそれだけは行った。
    単位はどうなろうとたいして影響なくなってしまったが、普段の数学に影響するのは避けたい。計測が多かったのでその待ち時間を上手く使うべきだった。
    どうしてもつい途中経過を見てしまう。不要なtweetしたりしたのもそうだが心が弱い。
    実行結果を眺めて出てくる方針だけで戦えたので今回は時間で殴りやすかったと言える気がする。
    考察よりの場合はどうするべきだろう。寝て起きると進むことが多いし多相睡眠に切り替えるのは可能性がある。
-   readme.mdに書くという方針は改善の余地がある。
    `git stash` を叩いたとき書きかけのreadme.mdまで持っていかれるの面倒だった。commit logが流れるし。
    gitでは管理せずissueに書くのが思い付く。しかし手元に残らないのが好きでないし、変に構造化されてしまって書きにくく、細切れになるため見返す頻度が下がるため不適。
    wikiはcode埋め込みなどがissueより低機能なので選択肢に入らない。
    別にgistを切ってそちらにやるのはありな気がするけど管理するものが増えてつらい。
    単にbranchを切って手元にふたつrepoを用意して進め最後にmergeするのはどうか、これなら良さそうだし次回試す。
-   計算資源借りて調整するのは正解だった。
    [task spooler](http://vicerveza.homeunix.net/~viric/soft/ts/)は便利だった。
    計測結果の管理が雑だったので改善したい。
    集計も上手くできていない。 sed grep awkで適当に処理しているが、現代的な道具を使うべきのはず。
    Kaggleの界隈ならこの手の知見持ってそうだし貰ってきてきたい。
-   計測の話やreadmeの話をまとめるに、必要なのはdashboard。
    コードを投げ付けたらいい感じに計測して表やグラフにしてくれるようなもの。
    ICFPCだと作ってる人が多いように思う: [@mecha_g3 さんの例](https://twitter.com/mecha_g3/statuses/894528840541773826)。
